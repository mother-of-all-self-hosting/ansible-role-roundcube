...
#
# Tags look like `v<Roundcube version>-<release>`:
#
# - if defaults/main.yml points at a Roundcube version that has never been
#   released, the release counter restarts at 0 (`v1.6.14-0`)
# - otherwise the counter is incremented (`v1.6.14-1`), but only if something
...
version="$(sed -nE 's|^roundcube_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Roundcube version from $defaults_path"
	exit 1
fi

# The version value carries no leading `v` (e.g. `1.6.14`) while the tags do,
# so one is added here — and not doubled, should the value ever gain one.
